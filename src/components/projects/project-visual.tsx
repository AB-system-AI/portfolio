...
import { getProjectCoverPath } from "@/lib/content/project-image-paths";
...
export function ProjectVisual({
  ...
  const coverFromContent = project.images[0];
  const coverPath = coverFromContent?.src ?? getProjectCoverPath(project.slug);
  const [useCover, setUseCover] = useState(Boolean(coverFromContent));
  ...
  useEffect(() => {
    if (coverFromContent) return;
    const img = new window.Image();
    img.onload = () => setUseCover(true);
    img.onerror = () => setUseCover(false);
    img.src = coverPath;
  }, [coverFromContent, coverPath]);
  ...
  if (useCover) {
    return (
      <div className="group/visual relative aspect-[16/10] overflow-hidden bg-muted">
        <Image
          src={coverPath}
          alt={coverFromContent?.alt ?? `${project.title} cover`}
          fill
          priority={priority}
  ...
  const content = (
    <>
      ...
      <div className="relative z-10 text-center">
        ...
        <p className="mt-1 text-sm font-medium opacity-60" style={{ color: accent }}>
    ...
    return (
      ...
      >
        <motion.div
          ...
          style={{ x: (tilt.rotateY / 4) * -1, y: (tilt.rotateX / 4) * -1 }}
          transition={springSoft}
        >
          {content}
        </motion.div>
